/*------------------------------------------------------------------------------
	...
	Programador tarea 2A: victor.fosch@estudiants.urv.cat
...
//Funcio propia de Víctor Fosch per testejar 2Ia al main
elemento getVectElem(int i)
{
	return vect_elem[i];
}
...
{	
	...
	for(int i=0; i < ROWS*COLUMNS; i++)
	{
		...
		if((mat[fila][columna] & 0x7) != 0x7 && (mat[fila][columna] & 0x7) != 0) //Màscara dels tres bits de menys pes
		{                                     									//per descartar posicions amb 7 o 15 o element buit
			crea_elemento(mat[fila][columna]&0x7,fila,columna);
			n_sprites++;
		}
	}
